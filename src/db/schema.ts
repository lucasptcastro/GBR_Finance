...
export const personSexEnum = pgEnum("person_sex", [
  "male", // masculino
  "female", // feminino
]);
...
export const peopleTable = pgTable("people", {
  ...
  category: personCategoryEnum("category").notNull(), // cliente ou fornecedor
  type: personTypeEnum("type").notNull(), // pessoa física ou jurídica
  sex: personSexEnum("sex").default("male"), // sexo
  isPcd: boolean("is_pcd").default(false), // pessoa com deficiência
  birthDate: timestamp("birth_date"), // data de nascimento
  rg: text("rg"), // RG
  stateRegistration: text("state_registration"), // inscrição estadual
  email: text("email"),
  phone: text("phone"), // contato/telefone fixo
  ...
  enrollmentStatus: enrollmentStatusEnum("enrollment_status")
    .default("enrolled")
    .notNull(),
  observation: text("observation"), // observação com limite de 2000 caracteres
  needsGuardian: boolean("needs_guardian").default(false).notNull(),
...
export const bankAccountsTable = pgTable("bank_accounts", {
  ...
  currentBalanceInCents: integer("current_balance_in_cents")
    .notNull()
    .default(0),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at")
...
export const eggSalesRelations = relations(eggSalesTable, ({ one }) => ({
  warehouse: one(warehousesTable, {
    fields: [eggSalesTable.warehouseId],
    references: [warehousesTable.id],
  }),
  creator: one(userTable, {
    fields: [eggSalesTable.createdBy],
    references: [userTable.id],
  }),
}));
